...
def ai_price_predict(df, current_price, selected_tf, n_future=5):
    ...
    try:
        ...
        future = prophet_model.make_future_dataframe(
            periods=n_future, freq=f'{selected_tf}min')
...
def fetch_ohlcv(market, timeframe, count=300):
    ...
    try:
        ...
        if len(df) > 16:
            ...
            df['MACD_hist'] = df['MACD_line'] - \
                df['MACD_line'].ewm(span=9, adjust=False).mean()
...
def get_current_prices(markets):
    ...
    try:
        res = requests.get(
            f"https://api.upbit.com/v1/ticker?markets={','.join(markets)}", timeout=10)
        res.raise_for_status()
        return {item['market']: {
...
def create_coin_chart(df, coin, tf_name):
    ...
    try:
        ...
        if 'HMA' in df.columns:
            fig.add_trace(go.Scatter(x=df['datetime'], y=df['HMA'], name='HMA', line=dict(
                color='blue')), row=1, col=1)
        if 'HMA3' in df.columns:
            fig.add_trace(go.Scatter(x=df['datetime'], y=df['HMA3'], name='HMA3', line=dict(
                color='orange')), row=1, col=1)
        if 'BB_upper' in df.columns:
            fig.add_trace(go.Scatter(x=df['datetime'], y=df['BB_upper'], name='BB Upper', line=dict(
                color='gray', dash='dot')), row=1, col=1)
        if 'BB_lower' in df.columns:
            fig.add_trace(go.Scatter(x=df['datetime'], y=df['BB_lower'], name='BB Lower', line=dict(
                color='gray', dash='dot')), row=1, col=1)
        ...
        if 'RSI' in df.columns:
            fig.add_trace(go.Scatter(x=df['datetime'], y=df['RSI'], name='RSI', line=dict(
...
def init_dashboard():
    # st.set_page_config(layout="wide")  # <-- 이 줄을 삭제 또는 주석 처리
    st.markdown("""
        <style>
...
def check_pattern_alerts(markets, timeframes, selected_tf):
    ...
    for market in markets:
        ...
        for pattern in wave_patterns:
            ...
            last_alert = st.session_state.last_alert_time.get(
...
def main():
    ...
    with st.sidebar:
        ...
        for coin in st.session_state.ai_progress:
            progress = min(
                st.session_state.ai_progress[coin] + random.randint(2, 6), 100)
    ...
    for alert in st.session_state.alerts[-10:]:
        ...
        st.markdown(
            f"<div class='{alert_type}'>{alert}</div>", unsafe_allow_html=True)
    ...
    for market in MARKETS:
        ...
        if market != 'KRW-STX':
            ...
            replace_qty = (
                default_holdings['KRW-STX'] * stx_price * 0.9995) / price
